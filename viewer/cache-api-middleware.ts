  ...
  try {
    ...
    const resolvedReal =
      typeof fs.realpathSync.native === "function"
        ...
        : fs.realpathSync(abs);
    if (
      resolvedReal !== repoReal &&
      !resolvedReal.startsWith(repoReal + path.sep)
    ) {
      return null;
    }
    return resolvedReal;
...
async function readJson(
  analysisPath: string,
): Promise<{ analyzedAt: string | null; clipCount: number } | null> {
  try {
    ...
    return {
      analyzedAt:
        typeof parsed.analyzedAt === "string" ? parsed.analyzedAt : null,
      clipCount,
    };
  } catch {
    return null;
...
export function createCacheApiMiddleware(
  ...
  return (req, res, next) => {
    ...
    if (parsed.pathname === "/api/runs" && req.method === "GET") {
      void (async () => {
        ...
        for (const dirent of entries) {
          ...
          const analysisPath = findRunFilePath(
            path.join(cacheDir, dirent.name),
            "**/vlm-analysis.json",
          );
    ...
    const runArtifactMatch =
      /^\/api\/run\/([^/]+)\/(analysis|shot-match|render-plan)$/.exec(
        parsed.pathname,
      );
    if (runArtifactMatch?.[1] && runArtifactMatch[2] && req.method === "GET") {
      ...
      const artifact = runArtifactMatch[2];
      const artifactPath =
        artifact === "analysis"
